...
CREATE TABLE partman.product_template (LIKE public.product INCLUDING ALL);

-- Add constraints or indexes to the template table
ALTER TABLE partman.product_template ADD CONSTRAINT product_category_id_unique UNIQUE (category_id);
...
CREATE MATERIALIZED VIEW mv_product_downloads_last_7d
AS SELECT p.category_id, p.product_id, COALESCE(rd.download_count, 0::bigint) AS download_count
   FROM product p
...
-- public.category_counts source

CREATE MATERIALIZED VIEW category_counts
TABLESPACE pg_default
AS SELECT category_id,
          count(*) AS count
   FROM product p
   WHERE status::text = 'publish'::text AND NOT (EXISTS ( SELECT 1
              FROM product_promo pp
              WHERE pp.product_id = p.product_id))
